  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch('http://localhost:3001/api/query3');
      ...
      <Table sx={{ minWidth: 650 }} aria-label="simple table">
        <TableHead>
          <TableRow>
            ...
            <TableCell style={{ width: '25%' , backgroundColor: 'black', color: 'white'}} align="right">Oral Cancer</TableCell>
            <TableCell style={{ width: '25%', backgroundColor: 'black', color: 'white' }} align="right">Diabetic</TableCell>
            <TableCell style={{ width: '25%' , backgroundColor: 'black', color: 'white'}} align="right">Disease, Cause and Prescription</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {data.map((item) => (
